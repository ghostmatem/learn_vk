...
class BaseAPIClient {
  ...
  Future<T> _getResponseResult <T>({
    ...
    final contentDecoded = await args.responseStreamDecoder(response);
    
    final result = args.responseContentParser(contentDecoded);

    final internalValidator = args.internalValidator;
    if (internalValidator != null) internalValidator(response, result);
